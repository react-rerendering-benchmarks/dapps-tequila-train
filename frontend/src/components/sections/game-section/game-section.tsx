...
export const GameSection = memo(() => {
  ...
  return <div className="container-xl flex flex-col grow">
      <ul className="space-y-px">
        <li>
          <div className={clsx('relative grid grid-cols-[170px_1fr] grid-rows-[36px] gap-10 py-3 px-3.5 rounded-lg', 'before:absolute before:inset-0 before:rounded-lg', 'before:bg-[#EBF1EE] before:bg-opacity-90', getBgColors(-1).backdrop)}>
        ...
        {state?.tracks.map((p, i) => <li key={i}>
            <PlayerTrackSection index={i} isUserTrain={p.hasTrain} active={+state?.currentPlayer === i} tiles={p.tiles} />
          </li>)}
      </ul>
